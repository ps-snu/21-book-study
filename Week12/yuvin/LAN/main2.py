...
def cal_dist_g(b_set1, b_set2, b_list):
    min_dist = float("INF")
    for b1 in b_set1:
        for b2 in b_set2:
...
def get_min_dist_list(num, g_dist_dict):
...
for _ in range(int(input())):
    num_tup = tuple(map(int,raw_input().split(" ")))
    b_num, c_num = num_tup[0], num_tup[1]
    b_list = list(
        zip_longest(
            *map(
                str_to_int_list,
